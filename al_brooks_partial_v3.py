"""
╔══════════════════════════════════════════════════════╗
║   🤖 AL BROOKS — PARTIAL TP v3 (FIXED)             ║
║                                                      ║
║   SL     : 1%                                       ║
║   TP1    : 1.25% → close 50% + SL ke entry         ║
║   TP2    : 2.5%  → close sisa 50%                  ║
║   RR     : 1:2.5                                    ║
...
def get_qty_info():
    try:
        r    = get_req("/v5/market/instruments-info", {"category": "linear", "symbol": PAIR})
        info = r["result"]["list"][0]["lotSizeFilter"]
        return float(info["minOrderQty"]), float(info["qtyStep"])
    except:
        return 0.001, 0.001

def hitung_qty(harga):
    mq, qs = get_qty_info()
    qty    = round(round((MARGIN_USD * LEVERAGE / harga) / qs) * qs, 10)
    return max(qty, mq)

def bulatkan_qty(qty):
    _, qs = get_qty_info()
    return max(round(round(qty / qs) * qs, 10), qs)
...
def set_leverage():
    try:
        post_req("/v5/position/set-leverage", {
            "category": "linear", "symbol": PAIR,
...
def pindah_sl_breakeven(entry, direction):
    try:
        be = round(entry * 1.0005, 2) if direction == "LONG" else round(entry * 0.9995, 2)
        r  = post_req("/v5/position/trading-stop", {
            "category": "linear", "symbol": PAIR,
            "stopLoss": str(be), "positionIdx": 0
        })
        if r["retCode"] == 0:
            log(f"🔒 SL → Breakeven ${be:,.2f} | Trade RISK FREE! 🎉")
            return True
        log(f"⚠️ Gagal BE: {r.get('retMsg','')}")
        return False
    except Exception as e:
        log(f"❌ Error BE: {e}"); return False

def partial_close(posisi, pct=0.5, label="TP1"):
    try:
        size  = float(posisi["size"])
        close = bulatkan_qty(size * pct)
        close = min(close, size)
        side  = "Sell" if posisi["side"] == "Buy" else "Buy"
        r     = post_req("/v5/order/create", {
            "category": "linear", "symbol": PAIR, "side": side,
            "orderType": "Market", "qty": str(close),
            "reduceOnly": True, "timeInForce": "GTC"
        })
        if r["retCode"] == 0:
            pnl = float(posisi["unrealisedPnl"]) * pct
            log(f"✅ {label} HIT! Close {int(pct*100)}% | Est PnL: ${pnl:+,.2f} | Sisa: ~{round(size-close,3)}")
            return True
        log(f"❌ Gagal {label}: {r.get('retMsg','')}")
        return False
    except Exception as e:
        log(f"❌ Error partial: {e}"); return False

def tutup_semua(posisi, label="TP2"):
    try:
        side = "Sell" if posisi["side"] == "Buy" else "Buy"
        r    = post_req("/v5/order/create", {
            "category": "linear", "symbol": PAIR, "side": side,
        ...
        log(f"{'✅ PROFIT' if pnl >= 0 else '❌ LOSS'} {label}! Semua ditutup | PnL: ${pnl:+,.2f}")
        reset_state()
    except Exception as e:
        log(f"❌ Error tutup: {e}")
...
def buka_posisi(direction, harga, alasan):
    ...
    if r["retCode"] == 0:
        ...
        log(f"   TP1   : ${tp1:,.2f} (+{TP1_PERSEN}%) → 50% + SL ke entry")
        log(f"   TP2   : ${tp2:,.2f} (+{TP2_PERSEN}%) → 50% sisa")
        log(f"   RR    : 1:{TP2_PERSEN/SL_PERSEN} | Lev: {LEVERAGE}x | Qty: {qty}")
        log(f"   Setup : {alasan}")
        state.update({
            "active": True, "direction": direction, "entry": harga,
        ...
        log(f"❌ Gagal buka: {r.get('retMsg','')}")

def monitor_tp(posisi, harga):
    if not state["active"]: return
    d, tp1, tp2, done = state["direction"], state["tp1"], state["tp2"], state["tp1_done"]

    if d == "LONG":
        if not done and harga >= tp1:
            log(f"🎯 TP1! ${harga:,.2f} >= ${tp1:,.2f}")
            if partial_close(posisi, 0.5, "TP1"):
                state["tp1_done"] = True
                pindah_sl_breakeven(state["entry"], d)
                state["be_done"] = True
                log(f"   Nunggu TP2 di ${tp2:,.2f}...")
        elif done and harga >= tp2:
            log(f"🎯 TP2! ${harga:,.2f} >= ${tp2:,.2f}")
            tutup_semua(posisi, "TP2")
...
def is_bull(c):
    return c["close"] > c["open"] and body_r(c) >= STRONG_BAR_MIN and close_p(c) >= 0.65

def is_bear(c):
    return c["close"] < c["open"] and body_r(c) >= STRONG_BAR_MIN and close_p(c) <= 0.35
...
def cek_failed_breakout(candles, trend):
    if len(candles) < 8: return None, ""
    lb         = candles[-8:-2]
    prev, last = candles[-2], candles[-1]
    rh = max(c["high"] for c in lb)
    rl = min(c["low"]  for c in lb)
...
def get_sinyal(candles):
    ...
    for fn, args in [
        (cek_2leg,            [candles, trend, ema]),
        (cek_failed_breakout, [candles, trend]),
        (cek_trend_bar,       [candles, trend, ema]),
    ]:
        s, a = fn(*args)
        if s: return s, ema, a
    return None, ema, ""
...
╔══════════════════════════════════════════════════════╗
║   🤖 AL BROOKS PARTIAL TP v3 — FIXED               ║
║   Pair: BTCUSDT | TF: M15 | Lev: 40x              ║
║   SL:1% | TP1:1.25%(50%) | TP2:2.5%(50%)          ║
║   RR: 1:2.5 | Demo Trading Bybit                   ║
╚══════════════════════════════════════════════════════╝
""", flush=True)
    ...
    while True:
        try:
            ...
            if posisi:
                ...
                t1   = "✅TP1+BE🔒" if state["tp1_done"] else f"⏳TP1@${state['tp1']:,.0f}"
                log(f"💹 ${harga:,.2f} | EMA:{ema:,.0f} | {trend} | {arah} PnL:${pnl:+,.2f} | {t1} → TP2@${state['tp2']:,.0f}")
